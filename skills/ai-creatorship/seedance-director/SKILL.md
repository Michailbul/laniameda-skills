...
description: >
  Seedance 2.0 scene direction API — converts plain-text scene descriptions into structured bilingual
  EN+ZH JSON video prompts optimized for Seedance 2.0. Handles action scenes (combat, pursuit, stunts),
  general scenes (landscapes, journeys, atmosphere), and dialogue scenes (confrontations, negotiations,
  interrogations) using archetype routing. Outputs pure JSON with no commentary. Use this skill whenever
  the user wants a production-ready Seedance prompt in JSON format, needs bilingual EN+ZH video prompts,
  describes an action/fight/chase/dialogue scene for Seedance, wants archetype-routed scene direction,
  or says "direct this scene", "JSON prompt", "bilingual prompt", "ZH prompt", or "seedance director".
  This skill is the structured-output counterpart to the other Seedance prompting skills — use it when
  the user needs machine-parseable JSON output rather than plain-text prompts, or when they need Chinese
  language prompts alongside English.
---
